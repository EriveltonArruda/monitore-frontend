// src/app/api/travel-expenses/[id]/advances/route.ts
...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const r = await fetch(`${API}/travel-expenses/${params.id}/advances`, { cache: 'no-store' });
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const r = await fetch(`${API}/travel-expenses/${params.id}/advances`, {
      method: 'POST',
